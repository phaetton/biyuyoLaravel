...
<div class=" rounded  pt-6   flex flex-col ">
  <div class="-mx-3 md:flex mb-6 justify-between">
    ...
    <div class="md:w-1/2 px-3">
      ...
      <input name="logo" value="{{old ('logo',$negocio->logo)}}" class="appearance-none block w-full bg-grey-lighter text-grey-darker border border-grey-lighter rounded py-3 px-4" id="grid-last-name" type="file" accept="image/*" placeholder="logo">
  ...
  <div class="md:w-1/2 px-3">
    ...
    <div class="relative">
      <select name="categoria" class="block appearance-none w-full bg-grey-lighter border border-grey-lighter text-grey-darker py-3 px-4 pr-8 rounded" id="grid-state">
        ...
        <option @if($negocio['categoria_id']==$categoria['id']) selected @endif value="{{$categoria['id']}}">
          {{$categoria['nombre']}}
        </option>
        @endforeach
      </select>
  ...
  <div class="md:w-1/2 px-3">
    ...
    <div class="relative">
      <select name="tiponegocio" class="block appearance-none w-full bg-grey-lighter border border-grey-lighter text-grey-darker py-3 px-4 pr-8 rounded" id="grid-state">
        ...
        <option @if($negocio['tiponegocio_id']==$tiponegocio['id']) selected @endif value="{{$tiponegocio['id']}}">{{$tiponegocio['nombre']}}</option>
        @endforeach
      </select>
      <div class="pointer-events-none absolute pin-y pin-r flex items-center px-2 text-grey-darker">
        <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
  ...
  <div class="md:w-1/2 px-3">
    ...
    <div class="relative">
      <select name="cliente" class="block appearance-none w-full bg-grey-lighter border border-grey-lighter text-grey-darker py-3 px-4 pr-8 rounded" id="grid-state">
        ...
        <option @if($negocio['cliente_id']==$cliente['id']) selected @endif value="{{$cliente['id']}}">{{$cliente['nombre']}}</option>
        @endforeach
      </select>
      <div class="pointer-events-none absolute pin-y pin-r flex items-center px-2 text-grey-darker">
        <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
...
<div class="-mx-3 md:flex mb-6">
  <div class="w-full px-3">
    ...
    <input name="email" value="{{$negocio->email}}" class="appearance-none block w-full bg-grey-lighter text-grey-darker border border-grey-lighter rounded py-3 px-4" id="grid-last-name" type="text" placeholder="email">
  </div>
