<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight flex items-center justify-between">
            ...
            <a href="{{route('imagenes.create')}}" class="px-4 py-2 rounded-lg text-white bg-gray-800 hover:bg-gray-900 font-bold  shadow-lg shadow-gray-200 transition ease-in-out duration-200 translate-10">subir</a>
                                ...
                                <div class="w-full text-slate-700 dark:text-white flex flex-col items-center">
                                    <div class=" text-left flex flex-col w-full items-center justify-center  md:border border-slate-300 dark:border-slate-600 rounded-xl">
                                        <div class="w-full rounded-xl flex-col xl:flex-row bg-white dark:bg-slate-900 shadow-md">
                                            <div class="rounded-t-xl w-full h-64 shadow-sm bg-cover" style='background-image: url({{$imagen->imagen}});'></div>

                                            <div class="w-full p-3 flex flex-col justify-between h-auto gap-4  lg:h-auto">
                                                <h1 class="text-left text-xs  font-bold leading-normal truncate">
                                                    {{$imagen->descripcion}}
                                                </h1>
                                                    ...
                                                    <form action="{{route('imagenes.destroy',$imagen)}}" method="POST" >
                                                        @csrf
                                                        @method('DELETE')
                                                        <input type="submit" value="Eliminar" onclick="return confirm('¿Desea eliminar?')" class="w-full px-4 py-2 rounded-lg text-white bg-gray-800 hover:bg-gray-900 font-bold  shadow-lg shadow-gray-200 transition ease-in-out duration-200 translate-10" />
                                                    </form>
                                                </div>
                                        </div>
                                    </div>
